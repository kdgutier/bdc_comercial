...
for (x in unique(com_data$unique_id)[1:3]){
  rm(fit)
  print(x)
...
top1<-subset(com_data, com_data$SKU==top1_sku & com_data$Subagencia==top1_age)
...
top1 <- ts(top1, frequency=12, start=c(2013,1))
decompose_top1 <- decompose(top1)
plot(decompose_top1)
library(forecast)
fit <- auto.arima(top1[,"Hectolitros"])
fit
accuracy(fit)
plot(forecast(fit, 1))

com_data2<-subset(com_data,com_data$hectolitros==0)
rand_sku <- sample(com_data2$sku,1)
rand_age <- sample(com_data2$subagencia,1)
rand_series <- subset(com_data2, ((com_data2$sku %in% rand_sku) & (com_data2$subagencia %in% rand_age) ))
rand_series <- ts(rand_series, frequency=12, start=c(2013,1))
rand_series <- rand_series[,"hectolitros"]
plot(rand_series)
fit <- auto.arima(rand_series)
fit
accuracy(fit)
#plot(fit)
plot(forecast(fit, 1)) 
